...
CLIENT_ARGS="--control-device /dev/bdr-1 --source-device /dev/mapper/bdr-1 --address 127.0.0.1 --port 9832 --no-print"
...
for i in {1..10}; do
    dd if=/dev/urandom of="$MAPPER_PATH" bs=4096 count=30 seek=$i conv=fsync &>/dev/null
